...
void ByteView::repositionCaret()
{
	...
	{
		if (((m_cursorAddr >= m_lines[i].address) && (m_cursorAddr < (m_lines[i].address + m_lines[i].length))) || (((i + 1) == m_lines.size()) && (m_cursorAddr == (m_lines[i].address + m_lines[i].length))))
		{
			if (i < m_topLine)
				m_topLine = i;
...
void ByteView::updateCaret()
{
	...
	{
		if (((m_prevCursorAddr >= m_lines[i].address) && (m_prevCursorAddr <= (m_lines[i].address + m_lines[i].length))) || ((m_cursorAddr >= m_lines[i].address) && (m_cursorAddr <= (m_lines[i].address + m_lines[i].length))))
		{
...
void ByteView::paintEvent(QPaintEvent* event)
{
	...
	for (int y = topY; y < botY; y++)
	{
		...
		int cursorCol = 0;
		if (((m_cursorAddr >= lineStartAddr) && (m_cursorAddr < (lineStartAddr + length))) || (((y + (int)m_topLine + 1) >= (int)m_lines.size()) && (m_cursorAddr == (lineStartAddr + length))))
		{
...
ByteViewType::ByteViewType() :
    ViewType("Bytes", "Byte Overview")
{
}
